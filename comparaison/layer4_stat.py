"""
Layer 4 – Statistical / Distribution Validation

All statistics computed in Python (pandas) because SQLite has no
built-in STDDEV or QUANTILE_CONT.
...
Checks (per numeric column):
  ...
  - Percentiles (P50, P95, P99 by default)

Plus:
  - Distribution confidence score (0–1)
"""
...
def run(db: SQLiteLoader, config: ValidationConfig) -> LayerResult:
    ...
    numeric_cols = [
        ...
        if pd.api.types.is_numeric_dtype(src_df[c]) or pd.api.types.is_numeric_dtype(tgt_df[c])
    ]
    ...
    if not numeric_cols:
        result.add(CheckResult(
            ...
            message="No numeric columns detected — statistical checks skipped.",
    ...
    t = time.perf_counter()
    all_stat_issues: list[dict] = []
    col_profiles: dict = {}

    for col in numeric_cols:
        src_series = pd.to_numeric(src_df[col], errors="coerce")
        tgt_series = pd.to_numeric(tgt_df[col], errors="coerce")

        src_stats = _compute_stats(src_series, src_count, config.percentiles)
        tgt_stats = _compute_stats(tgt_series, tgt_count, config.percentiles)

        col_issues = _compare_stats(col, src_stats, tgt_stats, config.stat_tolerance)
        all_stat_issues.extend(col_issues)
        col_profiles[col] = {
            "source": src_stats,
            "target": tgt_stats,
            "issues": col_issues,
    ...
    result.add(CheckResult(
        ...
        message=(
            ...
            if passed else
            f"Statistical deviation in {len(all_stat_issues)} stat(s) across {len(numeric_cols)} column(s)."
        ),
        details={
            "tolerance": config.stat_tolerance,
            "columns_checked": numeric_cols,
            "column_profiles": col_profiles,
            "issues": all_stat_issues,
        },
    ...
    # ── Null ratio ───────────────────────────────────────────────────
    t = time.perf_counter()
    null_issues = _check_null_ratios(src_df, tgt_df, common_cols,
                                     src_count, tgt_count, config.stat_tolerance)
    ...
    # ── Cardinality ──────────────────────────────────────────────────
    t = time.perf_counter()
    card_issues = _check_cardinality(src_df, tgt_df, common_cols, config.stat_tolerance)
    ...
    result.add(CheckResult(
        ...
        status=Status.PASSED if passed else Status.FAILED,
        message=(
            "Cardinality (distinct values) matches for all columns."
            if passed else
    ...
    result.add(CheckResult(
        ...
        status=Status.PASSED if passed else Status.FAILED,
        message=f"Distribution confidence score: {score:.1%} ({'≥' if passed else '<'} 95% threshold).",
...
# ─────────────────────────────────────────────
# Stat computation (all pandas — no SQLite ext)
# ─────────────────────────────────────────────
...
def _compute_stats(series: pd.Series, total_rows: int, percentiles: list[float]) -> dict:
    ...
    stats: dict = {
        ...
        "null_ratio": round(1 - len(clean) / total_rows, 6) if total_rows > 0 else 0,
    }
    for p in percentiles:
        key = f"p{int(p * 100)}"
        stats[key] = float(clean.quantile(p)) if len(clean) else None
    return stats
...
def _compare_stats(
    ...
    issues = []
    stat_keys = ["min", "max", "mean", "stddev"] + [k for k in src if k.startswith("p")]
...
def _check_null_ratios(
    ...
    issues = []
    for col in cols:
        sr = src_df[col].isna().sum() / src_count if src_count else 0
        tr = tgt_df[col].isna().sum() / tgt_count if tgt_count else 0
...
def _check_cardinality(
    ...
    issues = []
    for col in cols:
        sc = src_df[col].nunique()
        tc = tgt_df[col].nunique()
        if sc != tc:
            base = max(sc, 1)
...
def _confidence_score(col_profiles: dict) -> float:
    ...
    for data in col_profiles.values():
        n_stats = len([k for k in data.get("source", {})
                       if k not in ("non_null", "null_ratio")])
