"""
Validation Engine – Orchestrator  (pure pandas, no SQLite)

Flow:
  CSV  →  load_csv()  →  DataFrame  →  Layer 1..4  →  Report

Each layer receives (src_df, tgt_df, config) directly.
No database, no staging table, no extra dependencies.
...
Usage:
    ...
    config = ValidationConfig(
        ...
        aggregation_columns=["revenue"],    # optional
...
from core.normalize import load_csv
...
class ValidationEngine:
    ...
    def run(
        ...
        Args:
            source_df: Pre-loaded Talend DataFrame.
                       If None, loaded from config.talend_path.
            target_df: Pre-loaded SQL DataFrame.
                       If None, loaded from config.sql_path.
        """
        t_start = time.perf_counter()
        report  = ValidationReport.bootstrap(self.config)

        # ── Load & normalize ─────────────────────────────────────────
        print("[engine] Loading datasets …")
        try:
            ...
            print(f"[engine] {self.config.source_label}: "
                  f"{len(source_df):,} rows × {len(source_df.columns)} cols")
            print(f"[engine] {self.config.target_label}: "
                  f"{len(target_df):,} rows × {len(target_df.columns)} cols")
        ...
        l1 = self._safe_run(layer1_stuctural.run, "1_structural", source_df, target_df)
        ...
        l2 = self._safe_run(layer2_data.run, "2_data_level", source_df, target_df)
        report.layers.append(l2)

        # ── Layer 3: Business rules ──────────────────────────────────
        print("[engine] Layer 3: Business rules …")
        l3 = self._safe_run(layer3_bizRules.run, "3_business_rules", source_df, target_df)
        report.layers.append(l3)

        # ── Layer 4: Statistical ─────────────────────────────────────
        print("[engine] Layer 4: Statistical …")
        l4 = self._safe_run(layer4_stat.run, "4_statistical", source_df, target_df)
    ...
    def _safe_run(
        ...
        try:
            return layer_fn(src, tgt, self.config)
        except Exception as e:
...
if __name__ == "__main__":
    ...
    config = ValidationConfig(
        talend_path="data/testbench_output.csv",
        sql_path="data/sql_generated_output.csv",
        run_id="run_001",
        source_label="Talend",
        target_label="SQL",
        ...
        output_json_path="validation_report.json",
    )

    engine = ValidationEngine(config)
    report = engine.run(generate_narrative=False)